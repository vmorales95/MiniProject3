...
>>>>>>> d0a4d61392752a4dde97b5d86afab4d1e5783e59
```
```{r}
pre_47 <- this_big_thing %>%
  group_by(info) %>%
  filter(production_year < 1947) %>%
  summarize(genre_number = n()) %>%
  arrange(desc(genre_number))

<<<<<<< HEAD
```

```{r}
post_47 <- this_big_thing %>%
  group_by(info) %>%
  filter(production_year > 1947) %>%
  summarize(genre_number = n()) %>%
  arrange(desc(genre_number))
```
```{r}
multi_genre <- this_big_thing %>%
  group_by(title) %>%
  filter(production_year > 1947) %>%
  arrange(desc(total_genre))
=======
```{r}
